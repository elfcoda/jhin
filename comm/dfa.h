...
struct DFANode
{
    ...
    std::string toString()
    {
        ...
        for (const auto& item: followMap) {
            ...
            if (sSize < sMaxSize) {
                for (unsigned i = 0; i < sMaxSize - sSize; i++) s += "-";
            }
            /* format end */
    ...
    PNFA canReduce(unsigned token)
    {
        ...
        /* lookup shift-set */
        JHIN_ASSERT_BOOL(mEdges.find(token) != mEdges.end());
    ...
    while (!quDFAs.empty()) {
        ...
        for (PNFA p: s) {
            for (const auto& it: p->mNodes) {
                // unsigned -> unsigned(Syntax part), or char -> unsigned(Lex part)
                if (static_cast<unsigned>(it.first) != EP) {
