...
class Syntax
{
    public:
        ...
        std::unordered_set<unsigned> genFirstSetNonTerminal(unsigned id, std::unordered_map<unsigned, pSyntaxNFAData>& m)
        {
            ...
            while (!worklist.empty()) {
                ...
                JHIN_ASSERT_BOOL((productionIDs.find(i) != productionIDs.end()));
            ...
            while (!worklist.empty()) {
                ...
                if (p->position < size) {
                    ...
                    if (isNonTerminal(p->production[p->position])) {
                        JHIN_ASSERT_BOOL(productionIDs.find(p->production[p->position]) != productionIDs.end());
    ...
    private:
                                  ...
                                  std::unordered_set<unsigned>& nonTerminalHandling)
        {
            ...
            JHIN_ASSERT_BOOL(productionIDs.find(nonTerminal) != productionIDs.end());
        ...
        pSyntaxNFAData getSyntaxNFANode(unsigned nonTerminal, const std::vector<unsigned>& production, unsigned position)
        {
            ...
            JHIN_ASSERT_BOOL(SyntaxNFAData::mHash.find(p->hash) != SyntaxNFAData::mHash.end());
