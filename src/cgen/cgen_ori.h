...
{
    JHIN_ASSERT_BOOL(mCmpType2Instr.find(eType) != mCmpType2Instr.end());
...
{
    JHIN_ASSERT_BOOL(mLabelType2LabelBody.find(lbltype) != mLabelType2LabelBody.end());
...
std::string genLabel(ELabelType lbltype = E_LABEL_TYPE_OTHER)
{
    ...
    lblIdx += 1;
    JHIN_ASSERT_BOOL(lblIdx < ULLONG_MAX);
...
unsigned getSymbolIdx(const std::string& symbolName)
{
    ...
    if (ptr == nullptr) {
        ...
        JHIN_ASSERT_STR("symbol should be in this fn scope.");
    }
    return ptr->getIdx();
}
...
std::string genBool(const std::string& text)
{
    ...
    else JHIN_ASSERT_STR("Bool value error!");
...
{
    std::string s = "";
    JHIN_ASSERT_BOOL(pNode != nullptr && rtn1 != nullptr && rtn2 != nullptr);
    /* if-condition's branch */
    ast::pASTNode pIf = pNode->getChild(0);
    JHIN_ASSERT_BOOL(pIf != nullptr);
    ...
    if (ifConditionText == "True") {
        ...
        JHIN_ASSERT_STR("Not operator should be implemented first!");
    } else {
        ...
        if (ifConditionText == "==") {
            ...
            JHIN_ASSERT_STR("undefined syntax!");
        }
...
std::string genWhile(ast::pASTNode pNode, std::shared_ptr<st::symbolGenRtn> rtn1, std::shared_ptr<st::symbolGenRtn> rtn2)
{
    ...
    JHIN_ASSERT_BOOL(pWhile != nullptr);
...
{
    std::string s = "";
    JHIN_ASSERT_BOOL(pNode != nullptr && rtn1 != nullptr && rtn2 != nullptr && rtn3 != nullptr);
    /* if-condition's branch */
    ast::pASTNode pIf = pNode->getChild(0);
    JHIN_ASSERT_BOOL(pIf != nullptr);
    std::string ifConditionText = pIf->getText();
...
std::string genFnCall(const std::string& fnName, pTypeTree pArgsTree)
{
    ...
    for (int i = 0; i < pArgsTree->size(); i++) {
        ...
        if (argType == SYMBOL_TYPE_INT) {
            ...
            else JHIN_ASSERT_STR("invalid bool value!");
            s += "movq " + sValue + ", " + std::to_string(delta) + "(%rbp)\n";
        } else {
            // unsupported value
            JHIN_ASSERT_STR("unsupported value type!");
        }
    }
    s += "jmp " + fnLabel + "\n";
    return s;
