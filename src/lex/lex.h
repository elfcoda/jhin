...
class Lex
{
    public:
        ...
        pNFANode genNFA()
        {
            ...
            JHIN_ASSERT_BOOL(EKeyWords::FINAL_MARK - EKeyWords::START_MARK < MAX_KEY_WORDS - 5);
            JHIN_ASSERT_BOOL(static_cast<unsigned int>(EError::ERR_ERROR) > static_cast<unsigned int>(ERESymbol::RE_FINAL_MARK));

            /* gen initial node by node id 0 */
            pNFANode init = new NFANode(INITIALID);

            /* gen key words and symbols */
            for (unsigned i = 0; i < VKeyWords.size(); i++) {
