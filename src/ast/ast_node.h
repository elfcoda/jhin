...
struct ASTNodeData
{
    ...
    // maybe no longer needed as we use LLVM AST
    unsigned notation;
...
struct ASTNode
{
    ...
    std::string getNotationStr(unsigned idx)
    {
        ...
        JHIN_ASSERT_BOOL(syntax::id_to_non_terminal.find(notation) != syntax::id_to_non_terminal.end());
...
{
    std::string graphFile = "/Users/luwenjie/git/llvm-project/llvm/examples/Kaleidoscope/Jhin/jhin/src/ast/graph.py";
...
// outside of struct
std::string getSymbolString(pASTNode pNode)
{
    JHIN_ASSERT_BOOL(pNode != nullptr);
    return comm::symbolId2String(pNode->getSymbolId());
}
